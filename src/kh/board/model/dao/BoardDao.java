...
public class BoardDao {
	...
	public List<BoardVo> getBoardList(Connection conn){
		...
		try {
			...
			while(rs.next()) {
				...
				vo.setBoardOrigialFilename(rs.getString("board_Original_Filename"));
	...
	//
	public List<BoardVo> getBoardList(Connection conn, int srnum, int erunm){
		List<BoardVo> result = null;
		String sql = "select *"
				+ " from (select  rownum rn, tbl_1.*"
				+ "   from  (select  rownum xn, BOARD_NUM,BOARD_TITLE,BOARD_WRITER,BOARD_CONTENT,BOARD_ORIGINAL_FILENAME"
				+ "            ,BOARD_RENAME_FILENAME,BOARD_DATE,BOARD_LEVEL"
				+ "            ,BOARD_REF,BOARD_REPLY_SEQ,BOARD_READCOUNT  "
				+ "        from board"
